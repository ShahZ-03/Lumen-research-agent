...
from .groq_client import groq_stream_content
...
# PROMPT BUILDER
# =========================
def build_synthesis_prompt(chunks: List[Dict]) -> str | None:
    clean_chunks = []
    seen = set()
    ...
    for c in chunks:
        ...
        clean_chunks.append((content, source))

    # Limit chunks to avoid large prompts (prevents 429)
    clean_chunks = clean_chunks[:8]

    print(f"[Synthesizer] Using {len(clean_chunks)} chunks")
    ...
    context_blocks = [
        f"[Source {i+1}] ({src})\n{txt}"
        for i, (txt, src) in enumerate(clean_chunks)
    ]
...
async def synthesize_report(
    ...
    **kwargs: Any,
    ...
    if not chunks:
        return "No relevant information found to generate a report."

    prompt = build_synthesis_prompt(chunks)

    # Critical guard against empty context
    if not prompt:
        return "⚠️ No usable content found from retrieved sources. Try a more specific query."
...
async def generate(topic: str, job_id: str, vec, stream_queue=None):
    ...
    try:
        ...
        if not prompt:
            ...
            report = ""
            streamed_ok = False
            # True live streaming from Groq tokens.
            if stream_queue is not None:
                try:
            ...
            if not streamed_ok:
                report = await synthesize_report(chunks)
